...
{	
	if($selection)
	{
		...
		$properties = $filePerms->properties($id);
		
		 $sub['Html'] = array('action' => 'common.file.edit.wyswyg();','child' => false);
		 $sub['Text'] = array('action' => 'common.file.edit.text();','child' => false);
		 $sub['Source'] = array('action' => 'common.file.edit.code();','child' => false);
		 
		 $sub['Definitions'] = array('action' => 'common.file.defs();','child' => false);
		...
		if($filter == 'false')
		{
			if($filePerms->isErasable($id,$user)) $sub['Delete'] = array('action' => 'common.file.erase();','child' => false);
				...
				if($filePerms->isErasable($id,$user)) $sub['Delete'] = array('action' => 'common.entities.unlink();','child' => false);
					else $sub['Delete'] = array('action' => '','child' => false,'cssText' => 'color:#bababa;');
			}
		...
		*/	
		...
		//	else $items['Unlink'] = array('action' => 'common.entities.unlink();','child' => false);
		/*
...
{	
	if($selection)
	{
		...
		if($filter == 'false')
		{
			...
			if($folderPerms->isErasable($id,$user)) $sub['Delete'] = array('action' => 'common.entities.unlink();','child' => false);
				else $sub['Delete'] = array('action' => '','child' => false,'cssText' => 'color:#bababa;');
		}
...
else if($mode == 'empty')
{
	...
	$items['New'] = array('action' => '','child' => array('Folder' => $folder,'File' => $file));
		
	$items['Upload'] = array('action' => 'common.upload.create();','child' => false);
...
foreach($items as $index => $value)
{
		...
		if($value['child'])
		{
			...
			foreach($value['child'] as $index => $child)
			{
				...
				{
					$sub->id = 'sub';
					foreach($child['child'] as $index => $child2)
					{
						$sub2 = new Item($index);
						$sub2->action = $child2['action'];
						if(isset($child['cssText'])) $sub2->cssText = $child2['cssText'];
